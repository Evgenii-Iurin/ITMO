...
class NextAction(TypedDict):
    ...
    explaination: Annotated[str, ..., "Short explanaition why this aciton should be taken"]
...
class Agent():
    ...
    def _setup_agent(self):
        self.visited_map = np.full((self.area_size[0], self.area_size[1]), self.object_map.get("NOT_VISITED_AREA"))
    ...
    def update_visited_map(self, observation: np.ndarray, binary_observation_mask: np.ndarray) -> None:
        """
        ...
        if observation.shape != (mask_height, mask_width):
            raise ValueError("Observation shape does not match the size of the binary observation mask region.")
