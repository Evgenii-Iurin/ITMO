...
class Environment():
    ...
    def _generate_area(self):
        ...
        obstacles_x, obstacles_y = self.get_random_x_y(self.area_size[0], self.area_size[1], self.num_obstacles)        
        target_point_x, target_point_y = self.get_random_x_y(self.area_size[0], self.area_size[1], self.num_targets)
        ...
        for x, y in zip(obstacles_x, obstacles_y):
            ...
            self._insert_kernel(sampled_obstacle, (x, y), self.object_map.get("OBSTACLE"))
        
        # ========== Add targets ==========
        for x, y in zip(target_point_x, target_point_y):
            self._insert_kernel((3, 3), (x, y), self.object_map.get("TARGET_POINT"))
      
    def step(self, action: int, agent_position: tuple[int, int], observation_area: tuple[int, int]):
        if action == 0:  # up
    ...
    @staticmethod
    def get_random_x_y(x_max_size, y_max_size: int, num_of_samples: int) -> list[int] | int:
    ...
    def render_env(self, save_path):
        ...
        color_of_object: dict[int, str] = {object_representation: self.color_map[object] for object, object_representation in self.object_map.items()}
        for y in range(height):
            for x in range(width):
                value = self.area[y, x]
                color = color_of_object.get(value, 'black')  # Use 'black' as a fallback if value not in color_map
